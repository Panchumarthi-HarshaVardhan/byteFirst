...
/**
 * Reusable Card Front Face Content
 * Canonical single source of truth for both live 3D preview and Page 1 of print document.
 */
...
/**
 * Reusable Card Back Face Content
 * Canonical single source of truth for both live 3D preview and Page 2 of print document.
 */
...
/**
 * Dedicated Two-Page Print Document
 * Page 1: Front ID Card (centered, flat, un-mirrored)
 * Page 2: Back ID Card (centered, flat, un-mirrored)
 */
export function PrintDocument({ student, theme, orientation = 'vertical' }) {
  const {
    fullName,
    rollNumber,
    collegeName,
  ...
  return (
    <div
      className={`print-document ${isHorizontal ? 'print-horizontal' : 'print-vertical'}`}
      style={cardStyleVars}
      aria-hidden="true"
    >
      {/* PAGE 1: Front Side */}
      <div className="print-page print-page-front">
        <div
          ...
          className={`id-card-face id-card-front print-card-face ${isHorizontal ? 'face-horizontal horizontal' : ''}`}
          style={cardStyleVars}
        >
          <CardFrontContent
      ...
      <div className="print-page print-page-back">
        <div
          ...
          className={`id-card-face id-card-back print-card-face ${isHorizontal ? 'face-horizontal horizontal' : ''}`}
          style={cardStyleVars}
        >
          <CardBackContent
            displayCollege={displayCollege}
...
/**
 * Main Interactive IDCard Component
 * Renders the live 3D card preview on screen and includes the dedicated PrintDocument.
 */
const IDCard = forwardRef(function IDCard(
  ...
  return (
    <>
      {/* 1. Interactive 3D Card View (Screen View) */}
      <div className={`id-card-preview-wrapper id-card-perspective-container orientation-${orientation}`}>
        <div
          className={`id-card-visual id-card-root ${orientation} ${isFlipped ? 'is-flipped' : ''}`}
          style={cardStyleVars}
        >
          {/* CARD FRONT FACE (Single source of truth for export & live preview) */}
          <div
            ref={ref}
            id="downloadable-id-card"
            className={`id-card-face id-card-front printable-target ${isHorizontal ? 'face-horizontal horizontal' : ''}`}
            style={cardStyleVars}
          >
            <CardFrontContent
              displayName={displayName}
          ...
          {/* CARD BACK FACE (Interactive 3D Preview) */}
          <div
            className={`id-card-face id-card-back ${isHorizontal ? 'face-horizontal horizontal' : ''}`}
            style={cardStyleVars}
          >
            <CardBackContent
              displayCollege={displayCollege}
      ...
      {/* 2. Dedicated Print Document (Print & PDF Output: 2 Flat Pages) */}
      <PrintDocument student={student} theme={theme} orientation={orientation} />
    </>
  );
});
